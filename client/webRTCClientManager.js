...
class webRTCClientManager {
    ...
    init(roomObj) {
        this.signaling_socket = roomObj.socket;
        ...
        this.signaling_socket.on('connect', () => {
            ...
            this.setUpMedia(() => {
                ...
                joinChatRoom(() => {
                    ...
                    try {
                        this.signaling_socket.emit('webRTC_join', {roomCode: this.roomCode});
                    }
                    catch (error) {
        ...
        this.signaling_socket.on('addPeer', (config) => {
            ...
            peer_connection.ontrack = (event) => {
                ...
                try {
                    ...
                    this.attachMediaStream(remote_media, stream);
                }
                catch (error) {
                    // code that handles the error
                    console.error('An error occurred:', error.message);
                            ...
                            peer_connection.setLocalDescription(session_description,
                                () => { 
                                    this.signaling_socket.emit('relaySessionDescription', 
                                        {'peer_id': peer_id, 'session_description': session_description});
